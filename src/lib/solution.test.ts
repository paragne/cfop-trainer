...
describe("the video link", () => {
  ...
  const html = renderSolution(caseWith(["U"], url), "", 0);
...
describe("the algs", () => {
  it("lists every alg, primary first", () => {
    const html = renderSolution(caseWith(["R U R'", "F R F'", "L' U L"], null), "", 0);
    expect(html.startsWith('<p class="alg primary">')).toBe(true);
    expect(html.indexOf("R U R'")).toBeLessThan(html.indexOf("F R F'"));
  ...
  it("keeps parentheses and apostrophes readable", () => {
    expect(renderSolution(caseWith(["U' (R U R') [U2 R U' R']"], null), "", 0)).toContain(
      "U' (R U R') [U2 R U' R']",
    );
  });

  it("escapes markup in every alg, alternates included", () => {
    const html = renderSolution(caseWith(["R <b>&</b>", "L <i>"], null), "", 0);
...
describe("the multi-slot marker", () => {
  ...
  it("marks exactly the alternates that affect other slots, on their own line", () => {
    const html = renderSolution(flagged([false, true, false, true]), "", 0);
    expect(markers(html)).toBe(2);
    expect(html).toMatch(/R1 <span class="alg-flag"[^>]*>multi-slot<\/span><\/p>/);
    expect(html).toContain("R2</p>");
  });

  it("marks nothing when no alg is flagged", () => {
    expect(markers(renderSolution(flagged([false, false]), "", 0))).toBe(0);
  });
});

describe("with an AUF", () => {
  it("prefixes every alg, alternates included, merging a leading U turn", () => {
    const html = renderSolution(caseWith(["R U R'", "U R"], null), "U'", 0);
    expect(html).toContain(`U' R U R'</p>`);
    expect(html).toMatch(/>R<\/p>/);
  });

  it("changes nothing without one", () => {
    const c = caseWith(["U' (R U R')"], null);
    expect(renderSolution(c, "", 0)).toContain("U' (R U R')");
  });
});

describe("the star buttons", () => {
  const three = caseWith(["R U R'", "F R F'", "L' U L"], null);
  const buttons = (html: string) => [...html.matchAll(/<button [^>]*data-alg-star="(\d)" aria-pressed="(true|false)"[^>]*>(.)<\/button>/g)];

  it("puts one on every alg, numbered by its place in algs", () => {
    expect(buttons(renderSolution(three, "", 0)).map((m) => m[1])).toEqual(["0", "1", "2"]);
  });

  it.each([0, 1, 2])("marks exactly alg %i as starred", (starred) => {
    const pressed = buttons(renderSolution(three, "", starred)).map((m) => m[2] === "true");
    expect(pressed).toEqual([0, 1, 2].map((i) => i === starred));
  });

  it("fills the starred one and leaves the others outlined", () => {
    expect(buttons(renderSolution(three, "", 1)).map((m) => m[3])).toEqual(["☆", "★", "☆"]);
  });
});
